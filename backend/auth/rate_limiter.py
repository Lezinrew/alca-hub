...
class AdvancedRateLimiter:
    ...
    def _cleanup_expired_entries(self, key: str) -> None:
        ...
        while (self.storage[key] and 
               current_time - self.storage[key][0] > config.window_seconds):
            self.storage[key].popleft()
    
    async def check_rate_limit(self, identifier: str, rate_limit_type: RateLimitType) -> RateLimitResult:
    ...
    async def get_rate_limit_info(self, identifier: str, rate_limit_type: RateLimitType) -> Dict:
    ...
    async def reset_rate_limit(self, identifier: str, rate_limit_type: RateLimitType) -> bool:
...
class AdaptiveRateLimiter(AdvancedRateLimiter):
    ...
    async def get_adaptive_config(self, identifier: str, rate_limit_type: RateLimitType) -> RateLimitConfig:
        """Obter configuração adaptativa baseada no comportamento."""
        base_config = self.configs.get(rate_limit_type, self.configs[RateLimitType.GENERAL])
    ...
    async def check_adaptive_rate_limit(self, identifier: str, rate_limit_type: RateLimitType) -> RateLimitResult:
...
class DistributedRateLimiter(AdvancedRateLimiter):
    ...
    async def _get_redis_key(self, identifier: str, rate_limit_type: RateLimitType) -> str:
        """Gerar chave Redis para rate limiting."""
        return f"rate_limit:{identifier}:{rate_limit_type.value}"
    
    async def check_distributed_rate_limit(self, identifier: str, rate_limit_type: RateLimitType) -> RateLimitResult:
...
# Funções auxiliares
async def check_rate_limit(identifier: str, rate_limit_type: RateLimitType) -> RateLimitResult:
    """Verificar rate limiting para identificador."""
