...
RATE_LIMIT_PASSWORD_RESET = {
    ...
    "window_seconds": int(
        os.getenv("RATE_LIMIT_PASSWORD_RESET_WINDOW", "3600")
    ),  # 1 hora
}
...
BLACKLIST_TOKEN_LIFETIME = int(
    os.getenv("BLACKLIST_TOKEN_LIFETIME", "86400")
)  # 24 horas
BLACKLIST_CLEANUP_INTERVAL = int(
    os.getenv("BLACKLIST_CLEANUP_INTERVAL", "3600")
)  # 1 hora

# Configurações de Logs de Segurança
SECURITY_LOG_RETENTION_DAYS = int(os.getenv("SECURITY_LOG_RETENTION_DAYS", "30"))
SECURITY_LOG_CLEANUP_INTERVAL = int(
    os.getenv("SECURITY_LOG_CLEANUP_INTERVAL", "86400")
)  # 24 horas
...
SECURITY_NOTIFICATIONS = {
    "email_on_suspicious_activity": os.getenv(
        "EMAIL_ON_SUSPICIOUS_ACTIVITY", "true"
    ).lower()
    == "true",
    "email_on_multiple_logins": os.getenv("EMAIL_ON_MULTIPLE_LOGINS", "true").lower()
    == "true",
    "email_on_password_change": os.getenv("EMAIL_ON_PASSWORD_CHANGE", "true").lower()
    == "true",
}
...
MONITORING_METRICS_INTERVAL = int(
    os.getenv("MONITORING_METRICS_INTERVAL", "300")
)  # 5 minutos
MONITORING_ALERT_THRESHOLDS = {
    "high_error_rate": float(os.getenv("MONITORING_HIGH_ERROR_RATE", "0.05")),  # 5%
    "high_response_time": float(
        os.getenv("MONITORING_HIGH_RESPONSE_TIME", "2.0")
    ),  # 2 segundos
    ...
    try:
        ...
        for config_name, config in [
            ...
            if config["max_requests"] <= 0 or config["window_seconds"] <= 0:
                raise ValueError(
                    f"Configuração de rate limiting {config_name} inválida"
                )
