...
class AdvancedRateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            rate_limit_result = await self.rate_limiter.check_rate_limit(client_id, rule_name)
            ...
            response.headers["X-RateLimit-Strategy"] = rate_limit_result.strategy_used or "unknown"
...
class AdaptiveRateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _calculate_behavior_score(self, client_id: str):
        ...
        recent_requests = [
            req_time for req_time in behavior["requests"]
            if current_time - req_time < 3600
        ]
        
        # Analisar erros recentes (última hora)
        recent_errors = [
            err_time for err_time in behavior["errors"]
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        rate_limit_result = await self.rate_limiter.check_rate_limit(client_id, adaptive_rule)
        ...
        if not rate_limit_result.allowed:
            ...
            await self.security_manager.log_security_event(
                ...
                {
                    "rule": adaptive_rule,
                    "behavior_score": self.behavior_tracker.get(client_id, {}).get("behavior_score", 1.0),
                    "reason": rate_limit_result.reason
                }
            )
