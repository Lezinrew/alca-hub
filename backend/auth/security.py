...
class SecurityManager:
    ...
    async def _cleanup_blacklist(self):
        ...
        while True:
            current_time = time.time()
            expired_tokens = [
                token for token, timestamp in self.blacklist_storage
    ...
    async def check_rate_limit(self, request: Request, endpoint: str = "general") -> bool:
    ...
    async def log_security_event(self, event_type: str, user_id: Optional[str], 
                               request: Request, details: Dict = None) -> None:
    ...
    async def get_user_security_events(self, user_id: str, limit: int = 50) -> List[Dict]:
        ...
        events = await self.db.security_logs.find({
            "user_id": user_id
    ...
    async def detect_suspicious_activity(self, user_id: str) -> bool:
        ...
        recent_ips = await self.db.security_logs.distinct("ip_address", {
            "user_id": user_id,
        ...
        suspicious = (
            recent_login_attempts > 10 or  # Muitas tentativas de login
            len(recent_ips) > 3 or         # Múltiplos IPs
            denied_attempts > 5            # Muitas tentativas negadas
        )
